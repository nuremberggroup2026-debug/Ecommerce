export const categoriesListTheme = {
  main: "bg-[#f9f6ef] text-[#0c0a07]",
  heroSection: "mx-auto max-w-7xl px-6 py-24 lg:px-10",
  heroContent: "max-w-3xl",
  title: "font-serif text-5xl tracking-tight md:text-6xl",
  description: "mt-6 text-lg leading-relaxed text-stone-500",
  gridSection: "mx-auto max-w-7xl px-6 pb-24 lg:px-10",
  grid: "grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4",
  card: "group relative overflow-hidden rounded-sm aspect-square border border-[#b08d57]/15 shadow-sm transition-all duration-500 hover:shadow-xl",
  ...
  cardTitle: "font-serif text-2xl text-[#f9f6ef]",
  exploreLink: (isAr: boolean) =>
    `mt-2 flex items-center gap-2 text-sm text-[#d4b877]/90 opacity-0 transition-all duration-500 group-hover:opacity-100 ${
