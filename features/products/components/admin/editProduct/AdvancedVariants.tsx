...
export default function AdvancedVariants({
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useFormContext<UpdateProductFormType>();

  const buildValueToAttributeMap = (attrs: AttributesWithValues[]) => {
    const map: Record<string, string> = {};
    attrs.forEach((attr) => {
      attr.attributeValues.forEach((v) => {
        map[v.attributeValueId] = attr.attributeId;
      });
    });
    return map;
  };
  ...
  >(() => {
    const valueToAttribute = buildValueToAttributeMap(attributesWithValues);
    const seeded: Record<number, Record<string, string>> = {};

    fields.forEach((field, index) => {
      const ids = (field as any).attributeValueIds as string[] | undefined;
      if (!ids?.length) return;
      seeded[index] = {};
      ids.forEach((valueId) => {
        const attributeId = valueToAttribute[valueId];
        if (attributeId) seeded[index][attributeId] = valueId;
      });
    });
  ...
  >(() => {
    ...
    fields.forEach((field, index) => {
      const url = (field as any).variantImage as string | undefined;
      if (url) seeded[index] = url;
    });
    return seeded;
  });
  const [replacingImageAt, setReplacingImageAt] = useState<Set<number>>(
    new Set(),
  );
