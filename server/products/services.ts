...
export const updateProductWithVariant = async (
  ...
  try {
    ...
    const variantIdsInUpdateForm = variants
      .map((v) => v.id)
      .filter((id): id is string => id !== undefined);
    const deleteVariantIds = currentProduct.productVariants
      .filter((v) => !variantIdsInUpdateForm.includes(v.id))
      .map((v) => v.id);
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (variants && variants.length > 0) {
        ...
        if (variants && variants.length > 0) {
          for (const variant of variants) {
            ...
            if (variantId) {
              ...
              if (!existedVariant) {
                throw new Error(`VARIANT_NOT_FOUND_${variantId}`);
              }
              ...
              console.log(
                `number: ${variant.discountPercentage}: ${finalPrice}, ${arrayOfFinalPrice}`,
              );
            ...
            // New variant
              ...
              const discountPercentage = variantData.discountPercentage ?? 0;
        ...
        console.log("arrayOfFinalPrice: ", arrayOfFinalPrice);

        console.log(
          "new Prisma.Decimal(Math.min(...arrayOfFinalPrice)): ",
          new Prisma.Decimal(Math.min(...arrayOfFinalPrice)),
        );
...
const getCachedProductBySlugAndLocale = async (slug: string, locale: Locale) =>
  unstable_cache(
    async () => {
    ...
    [`product-by-slug-${slug}-and-locale-${locale}`],
    {
...
export const getProductBySlugAndLocale = async (
  locale: Locale,
  slug: string,
  userId?: string,
) => {
  const [product, wishlist, cartItems] = await Promise.all([
    getCachedProductBySlugAndLocale(slug, locale),
    ...
    prisma.cart_items.findMany({
      where: { cart: { userId }, productVariants: { products: { slug } } },
      select: {
        variantId: true,
        quantity: true,
      },
